...
由于 volatile 变量只能保证可见性，在不符合一下两条规则的运算场景中，仍然要通过枷锁（synchronized、java.util.concurrent 中的锁或原子类）来保证原子性: 1) 运算结果并不依赖变量的当前值，或能够确保只有单一的线程修改变量的值; 2) 变量不需要与其他的状态变量共同参与不变约束;
